...
fn rmk_native_key_actions_supported(dev_conn: &crate::hid::HidDevice) -> anyhow::Result<bool> {
    match dev_conn.get_rmk_native_capabilities() {
        Ok(capabilities) => Ok(capabilities.key_actions),
    ...
    fn start_connect_with_reconnect(
        ...
        std::thread::spawn(move || {
            ...
            let result = (|| -> Result<ConnectResult, String> {
                ...
                let supports_rmk_native_key_actions = rmk_native_key_actions_supported(&dev_conn)
                    .map_err(|error| {
...
mod tests {
    ...
    fn disconnect_after_initial_keymap_read_fails_rmk_capability_probe() {
        ...
        let error = rmk_native_key_actions_supported(&device).unwrap_err();
    ...
    fn timeout_after_initial_keymap_read_keeps_rmk_capability_probe_optional() {
        ...
        assert!(!rmk_native_key_actions_supported(&device).unwrap());
    }
